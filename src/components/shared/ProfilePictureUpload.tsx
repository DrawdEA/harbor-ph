...
export default function ProfilePictureUpload({ 
  ...
  const getInitials = (name: string): string => {
    ...
    if (parts.length === 0) return type === 'user' ? 'U' : 'O';
  ...
  const getDefaultIcon = () => {
    if (icon) return icon;
    return type === 'user' ? <User className="w-12 h-12 text-muted-foreground" /> : <Building2 className="w-12 h-12 text-muted-foreground" />;
  };
    ...
    <Card className="font-roboto border-muted bg-background flex w-full flex-col overflow-hidden rounded-md border shadow-sm">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          {type === 'user' ? 'Profile Picture' : 'Organization Logo'}
        </CardTitle>
      </CardHeader>
        ...
        <div className="flex items-center gap-6">
          <div className="relative">
            {previewUrl ? (
              <div className="w-[100px] h-[100px] rounded-full overflow-hidden border-2 border-muted">
                <Image
                  src={previewUrl}
                  alt={`${fullName}'s ${type === 'user' ? 'profile picture' : 'logo'}`}
                  width={100}
                  height={100}
          ...
          <div className="flex-1">
            <h3 className="font-medium mb-2">
              {type === 'user' ? 'Current Picture' : 'Current Logo'}
            </h3>
            <p className="text-sm text-muted-foreground">
              {previewUrl 
                ? `You have a ${type === 'user' ? 'profile picture' : 'logo'} set` 
                : `No ${type === 'user' ? 'profile picture' : 'logo'} set`
              }
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div>
            <label className="text-sm font-medium">
              Upload New {type === 'user' ? 'Picture' : 'Logo'}
            </label>
          ...
          {success && (
            <div className="p-3 bg-green-500/10 border border-green-500/20 rounded-md">
              <p className="text-sm text-green-600">
                {type === 'user' ? 'Profile picture' : 'Organization logo'} updated successfully!
              </p>
            </div>
          )}
        </div>
